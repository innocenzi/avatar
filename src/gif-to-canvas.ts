...
class Player {
	...
	putFrame() {
		...
		this.options.tmpCanvas
			.getContext('2d')
			?.putImageData(this.options.frames[this.i].data, offset.x, offset.y)
	...
	step() {
		...
		function doStep(this: Player) {
			...
			if (nextFrameNo === 0) {
				...
				timer = setTimeout(() => doStep.apply(this), delay)
			}
		}
		...
		return new Promise<any>((resolve, reject) => {
			...
			xhr.onload = (e: any) => {
				if (e.target.status !== 200 && e.target.status !== 304) {
					reject(new Error(`Status Error: ${e.target.status}`))
					return
				}
