	...
	return (function e(t, n, r) {
		...
		1: [function (require, module, exports) {
			function EventEmitter() {
				...
				let er, handler, len, args, i, listeners; if (!this._events) {
					...
					if (!this._events.error || isObject(this._events.error) && !this._events.error.length) {
						er = arguments[1]; if (er instanceof Error) {
							...
							const err = new Error(`Uncaught, unspecified "error" event. (${er})`); err.context = er; throw err
						}
					}
					...
					switch (arguments.length) {
						case 1: handler.call(this); break; case 2: handler.call(this, arguments[1]); break; case 3: handler.call(this, arguments[1], arguments[2]); break; default: args = Array.prototype.slice.call(arguments, 1); handler.apply(this, args)
					}
				...
				let list, position, length, i; if (!isFunction(listener)) {
					...
					return this
				} list = this._events[type]; length = list.length; position = -1; if (list === listener || isFunction(list.listener) && list.listener === listener) {
		...
		3: [function (require, module, exports) {
			let EventEmitter; let GIF; let browser; const extend = function (child, parent) {
				...
				return -1
			}; const slice = [].slice; EventEmitter = require('events').EventEmitter; browser = require('./browser.coffee'); GIF = (function (superClass) {
				let defaults, frameDefaults; extend(GIF, superClass); defaults = { workerScript: 'gif.worker.js', workers: 2, repeat: 0, background: '#fff', quality: 10, width: null, height: null, transparent: null, debug: false, dither: false }; frameDefaults = { delay: 500, copy: false }; function GIF(options) {
					let base, key, value; this.running = false; this.options = {}; this.frames = []; this.freeWorkers = []; this.activeWorkers = []; this.setOptions(options); for (key in defaults) {
					...
					let key, results, value; results = []; for (key in options) {
						if (!hasProp.call(options, key)) {
							continue
						} value = options[key]; results.push(this.setOption(key, value))
					}

					return results
				}; GIF.prototype.addFrame = function (image, options) {
					let frame, key; if (options == null) {
						options = {}
					...
					let i, j, numWorkers, ref; if (this.running) {
						throw new Error('Already running')
					} if (this.options.width == null || this.options.height == null) {
						throw new Error('Width and height must be set prior to rendering')
					} this.running = true; this.nextFrame = 0; this.finishedFrames = 0; this.imageParts = (function () {
						let j, ref, results; results = []; for (i = j = 0, ref = this.frames.length; ref >= 0 ? j < ref : j > ref; i = ref >= 0 ? ++j : --j) {
							results.push(null)
						}
					...
					let j, numWorkers, ref, results; numWorkers = Math.min(this.options.workers, this.frames.length); (function () {
						results = []; for (let j = ref = this.freeWorkers.length; ref <= numWorkers ? j < numWorkers : j > numWorkers; ref <= numWorkers ? j++ : j--) {
							results.push(j)
						}
						...
						return function (i) {
							let worker; _this.log(`spawning worker ${i}`); worker = new Worker(_this.options.workerScript); worker.onmessage = function (event) {
								_this.activeWorkers.splice(_this.activeWorkers.indexOf(worker), 1); _this.freeWorkers.push(worker)
					...
					let i, j, ref; this.log(`frame ${frame.index} finished - ${this.activeWorkers.length} active`); this.finishedFrames++; this.emit('progress', this.finishedFrames / this.frames.length); this.imageParts[frame.index] = frame; if (this.options.globalPalette === true) {
						this.options.globalPalette = frame.globalPalette; this.log('global palette analyzed'); if (this.frames.length > 2) {
							for (i = j = 1, ref = this.freeWorkers.length; ref >= 1 ? j < ref : j > ref; i = ref >= 1 ? ++j : --j) {
								this.renderNextFrame()
							}
						}
					...
					let data, frame, i, image, j, k, l, len, len1, len2, len3, offset, page, ref, ref1, ref2; len = 0; ref = this.imageParts; for (j = 0, len1 = ref.length; j < len1; j++) {
					...
					let frame, task, worker; if (this.freeWorkers.length === 0) {
						throw new Error('No free workers')
					} if (this.nextFrame >= this.frames.length) {
						return
					} frame = this.frames[this.nextFrame++]; worker = this.freeWorkers.shift(); task = this.getTask(frame); this.log(`starting frame ${task.index + 1} of ${this.frames.length}`); this.activeWorkers.push(worker)

					return worker.postMessage(task)
				}; GIF.prototype.getContextData = function (ctx) {
					return ctx.getImageData(0, 0, this.options.width, this.options.height).data
				}; GIF.prototype.getImageData = function (image) {
					let ctx; if (this._canvas == null) {
						this._canvas = document.createElement('canvas'); this._canvas.width = this.options.width; this._canvas.height = this.options.height
					} ctx = this._canvas.getContext('2d'); ctx.setFill = this.options.background; ctx.fillRect(0, 0, this.options.width, this.options.height); ctx.drawImage(image, 0, 0)
					...
					let args; args = arguments.length >= 1 ? slice.call(arguments, 0) : []; if (!this.options.debug) {
						return
					}
